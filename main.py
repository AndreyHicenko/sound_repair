...
class Mydialog(StatesGroup):
    state_sign_up_mobile_number_repair = State()
    state_sign_up_name_repair = State()
...
# 2//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
@dp.callback_query_handler(text='callback_acoustics_repair')
async def callback_repair(callback: types.callback_query, state: FSMContext):
    await bot.delete_message(callback.from_user.id, callback.message.message_id)
    await bot.send_message(callback.from_user.id, '<b>Укажите как к вам обращаться</b>', parse_mode='HTML',
                           reply_markup=kb_sign_up_only_back)
    await Mydialog.state_sign_up_name_repair.set()


@dp.callback_query_handler(text='callback_sign_up_back', state=Mydialog.state_sign_up_name_repair)
async def callback_sign_up_back(callback: types.callback_query, state: FSMContext):
    await state.finish()
    await bot.delete_message(callback.from_user.id, callback.message.message_id)
    await bot.send_message(callback.from_user.id, '<b>Выберете куда вы хотите записаться</b>', parse_mode='HTML',
...
@dp.message_handler(state=Mydialog.state_sign_up_name_repair)
async def callback_repair_state(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['text'] = message.text
        user_message = data['text']
        (await update_name_users(user_message, message.from_user.id))
        ...
        await bot.send_message(message.from_user.id, f"<b>Вас зовут</b> "
                                                     f"{user_message}?", parse_mode='HTML',
                               reply_markup=kb_sign_up_name_repair)


@dp.callback_query_handler(text='callback_repair_no', state=Mydialog.state_sign_up_name_repair)
async def callback_repair_no(callback: types.callback_query, state: FSMContext):
    await state.finish()
    await bot.delete_message(callback.from_user.id, callback.message.message_id)
    await bot.send_message(callback.from_user.id, '<b>Укажите как к вам обращаться</b>', parse_mode='HTML',
                           reply_markup=kb_sign_up_only_back)
...
@dp.callback_query_handler(text='callback_repair_yes', state=Mydialog.state_sign_up_name_repair)
async def callback_car_yes(callback: types.callback_query, state: FSMContext):
    await state.finish()
    await bot.delete_message(callback.from_user.id, callback.message.message_id)
    await bot.send_message(callback.from_user.id, '<b>Укажите ваш номер телефона в формате'
                                                  ' 79XXXXXXXXX</b>', parse_mode='HTML',
                           reply_markup=kb_sign_up_only_back_num)
    await Mydialog.state_sign_up_mobile_number_repair.set()


@dp.message_handler(state=Mydialog.state_sign_up_mobile_number_repair)
async def callback_car_state(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['text'] = message.text
        user_message = data['text']
...
@dp.callback_query_handler(text='callback_sign_up_installation_num_yes',
                           state=Mydialog.state_sign_up_mobile_number_repair)
async def callback_installation_yes(callback: types.callback_query, state: FSMContext):
    await state.finish()
    await bot.delete_message(callback.from_user.id, callback.message.message_id)
    btn_resend_users = InlineKeyboardButton(text='Перезвонить',
    ...
    kb_admin_install = InlineKeyboardMarkup()
    kb_admin_install.add(btn_resend_users)
    await bot.send_message((await get_admin_users_with_role('acoustics_repair')),
                           f'<b>{(await get_users_name(callback.from_user.id))} записался(лась) на ремонт акустики.\n\n'
                           f'перезвоните ему(ей) по номеру</b> {(await get_users_number_phone(callback.from_user.id))}',
                           reply_markup=kb_admin_install, parse_mode='HTML')
    await bot.send_message(callback.from_user.id, f'<b>Вы записались на ремонт акустики.'
                                                  f' В ближайшее время вам перезвонят по указаному вами номеру </b>'
                                                  f'{(await get_users_number_phone(callback.from_user.id))}'
                                                  f' <b>для уточнения времени</b>'
                           , parse_mode='HTML',
...
@dp.callback_query_handler(text='callback_sign_up_back_num', state=Mydialog.state_sign_up_mobile_number_repair)
async def callback_sign_up_back_(callback: types.callback_query, state: FSMContext):
    await state.finish()
    await bot.delete_message(callback.from_user.id, callback.message.message_id)
    await bot.send_message(callback.from_user.id, '<b>Выберете куда вы хотите записаться</b>', parse_mode='HTML',
